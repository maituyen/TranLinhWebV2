...
(function ($) {
    ...
    var _validate =async function (wiz, settings) {
    ...
    var _move =async function (wiz, next) {
        var settings = wiz.data(_dataKey);

        // special handing on next
        if (next === true) {
            var x = await _validate(wiz, settings);
    ...
    var _initMarkupButtons = function (settings) {
        ...
        if (settings.showCancel === true) {
            html.push('<button type="button" class="btn btn-danger wizard-cancel">Hủy</button>');
        }
        html.push('<button type="button" class="btn btn-primary wizard-ok">Quay về danh sách</button>');
    ...
    var _setProgress = function (progress, stepIndex, stepCount) {
        ...
        progress.html('Bước ' + stepIndex + ' / ' + stepCount);
